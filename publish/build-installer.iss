...
OutputDir=artifacts
OutputBaseFilename=setup-vicon-{#MyAppVersion}-win-x64
...
begin
  if not IsDotNetInstalled('Microsoft.NETCore.App 10.0.') then
  begin
    MsgBox('Please install the .NET 10 Runtime to run this application.', mbError, MB_OK);
    Result := False;
  end
  else
  begin
